...
namespace Knacka.Se.ProtobufGenerator
{
    public class GenerateFromProto : ICanGenerateFromProto
    {
        ...
        /*
        public byte[] GenerateCsharpFromProto(string protoContent, string protoDirPath, string name)
        {
        ...
        */
        public byte[] GenerateCsharpFromProto(string protoContent, string protoDirPath, string name)
        {
            ...
            var infile = protoDirPath + name;
            string outdir = protoDirPath;

            string outputPath = protoDirPath + Path.GetFileNameWithoutExtension(infile) + ".cs";
            if (File.Exists(outputPath))
            {
                File.Delete(outputPath);
            }

            var exitCode = RunProtoc(_protocPath, $"--csharp_out={outdir} --proto_path={protoDirPath} {name}", protoDirPath, out stdout, out stderr);
