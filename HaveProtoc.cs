...
namespace Knacka.Se.ProtobufGenerator
{
    public class HaveProtoc : IHaveProtoc
    {
        ...
        {
            var path = FindParentsTools("protoc.exe");
            if (!string.IsNullOrEmpty(path))
            {
                return path;
            }

            path = FindExePath("protoc.exe");
        ...
        private string FindParentsTools(string exe)
        {
            DirectoryInfo current = new DirectoryInfo(_localFilepath);

            while (current != null && current.Exists)
            {
                string toolsPath = current.FullName + "\\Tools";
                if (Directory.Exists(toolsPath))
                {
                    string exePath = toolsPath + "\\" + exe;
